                ...
                <div key={t.id} className={`rounded-[2rem] border transition-all ${isSelected ? "bg-emerald-50 border-emerald-300 shadow-lg" : "bg-white border-slate-200 shadow-sm hover:border-slate-300"}`}>
                  <div className="p-6 space-y-6">
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                      ...
                      {t.hint && <button onClick={() => onToggleHint(t.id)} className="px-4 py-2 rounded-2xl bg-slate-100 text-slate-700 text-[10px] font-black hover:bg-slate-200 transition-all">Подсказка</button>}
                      {t.solution && <button onClick={() => onToggleSolution(t.id)} className="px-4 py-2 rounded-2xl bg-slate-100 text-slate-700 text-[10px] font-black hover:bg-slate-200 transition-all">Решение</button>}
