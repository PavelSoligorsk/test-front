...
export default function TestsTab({
  ...
  // Функция для проверки, содержит ли название раздела экзаменационные ключевые слова
  ...
  // Фильтрация разделов в зависимости от examFilter
        ...
        <div className="w-full flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
            <button 
              ...
              className={`px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all duration-300 active:scale-95 flex items-center justify-center gap-1.5 border select-none shrink-0 ${
                examFilter 
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl md:rounded-[2.5rem] shadow-sm border border-slate-100/80 dark:border-slate-700 overflow-hidden flex flex-col md:flex-row min-h-[450px] md:min-h-[550px]">
        ...
        <aside className={`w-full md:w-64 bg-slate-50/60 dark:bg-slate-900/60 border-b md:border-b-0 md:border-r border-slate-100 dark:border-slate-700 p-4 md:p-5 flex flex-col gap-3 ${selectedClass ? 'hidden md:flex' : 'flex'}`}>
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-slate-500 bg-white dark:bg-slate-700 px-2 py-0.5 rounded-md border border-slate-100 dark:border-slate-600 shadow-sm">
              ...
              return (
                <button 
                  ...
                  className={`w-full flex items-center justify-between p-3 rounded-xl text-left transition-all border ${
                    isActive 
                      ? 'bg-slate-800 dark:bg-slate-700 text-white border-transparent shadow-md shadow-slate-700/20' 
                      : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-100 dark:border-slate-700 hover:border-slate-200 hover:bg-slate-50/50 dark:hover:bg-slate-700/50'
                  }`}
                >
                  <span className="font-extrabold text-xs uppercase truncate mr-2 flex items-center gap-1.5">
                    {cls}
                    {isExam && (
                      <span className="text-[8px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded font-black shrink-0">
                        ЦТ/ЦЭ
                      </span>
                    )}
                  </span>
              ...
              <div className="flex flex-col gap-3 pb-4 border-b border-slate-100 dark:border-slate-700">
                <div className="flex items-center gap-1.5 flex-wrap text-[10px] md:text-xs font-black uppercase text-slate-400">
                  ...
                  <span className="bg-blue-50 text-blue-600 px-2 py-1 rounded-md">
                    {selectedClass}
                    {isExamClass(selectedClass) && ' 📝'}
                  </span>
                  
                  {selectedSubject !== 'Все' && (
                    <>
                      <ChevronRight size={12} className="text-slate-300" />
                      <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded-md">{selectedSubject}</span>
                    </>
                  )}
                </div>
                      ...
                      <button 
                        ...
                        className={`px-3.5 py-2 rounded-xl text-[10px] md:text-xs font-black uppercase whitespace-nowrap transition-all border ${
                          selectedSubject === subject 
                            ? 'bg-slate-800 dark:bg-slate-600 text-white border-transparent shadow-sm' 
                            : 'bg-slate-50 dark:bg-slate-700 text-slate-500 dark:text-slate-300 border-slate-100 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-600'
                        }`}
                      >
