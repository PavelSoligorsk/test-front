...
export default function AiModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-800 rounded-[2rem] shadow-2xl max-w-lg dark:border dark:border-slate-700 w-full overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <textarea value={aiPrompt} onChange={e => setAiPrompt(e.target.value)}
              ...
              className="w-full p-4 bg-slate-50 dark:bg-slate-700 rounded-2xl font-medium text-sm h-32 dark:text-white resize-none outline-none focus:ring-2 focus:ring-purple-400 transition-all" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" min="1" max="30" value={aiTaskCount}
                ...
                className="w-full p-3 bg-slate-50 dark:bg-slate-700 rounded-xl font-bold text-sm outline-none dark:text-white dark:placeholder:text-slate-400 focus:ring-2 focus:ring-purple-400 transition-all" />
            </div>
            <div>
              ...
              <select value={aiDifficulty} onChange={e => setAiDifficulty(e.target.value)}
                className="w-full p-3 bg-slate-50 dark:bg-slate-700 rounded-xl font-bold text-sm outline-none dark:text-white dark:placeholder:text-slate-400">
          ...
          <div className="bg-amber-50 dark:bg-amber-900/30 border border-amber-100 dark:border-amber-800 p-4 rounded-2xl flex items-start gap-3">
            ...
            <p className="text-[10px] font-bold text-amber-700 dark:text-amber-300 uppercase">AI может допускать ошибки. Проверяйте сгенерированные задания.</p>
          </div>
        </div>
        <div className="p-6 bg-slate-50 dark:bg-slate-700 flex gap-3">
          <button onClick={() => setShowAiModal(false)}
            className="flex-1 p-4 bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-2xl font-black text-sm hover:bg-slate-100 dark:hover:bg-slate-600 transition-all border border-slate-200 dark:border-slate-600">ОТМЕНА</button>
