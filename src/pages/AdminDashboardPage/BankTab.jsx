...
export default function BankTab({ tasks, groupedTasks, availableClasses, bankClass, setBankClass, bankTopic, setBankTopic, onEditTask, onTasksUpdate }) {
  ...
  const topicsForClass = useMemo(() => {
    if (!bankClass || !groupedTasks[bankClass]) return [];
    return Object.keys(groupedTasks[bankClass]).sort();
  }, [bankClass, groupedTasks]);
  ...
  const currentTasks = useMemo(() => {
    if (!bankTopic || !bankClass || !groupedTasks[bankClass]?.[bankTopic]) return [];
    let list = groupedTasks[bankClass][bankTopic];
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {filteredClasses.map(cls => {
                              const topicsCount = Object.keys(groupedTasks[cls] || {}).length;
                              const totalTasks = Object.values(groupedTasks[cls] || {}).reduce((sum, arr) => sum + (arr?.length || 0), 0);
                              return (
