      ...
      components={{
        ...
        img: ({ node, src, alt, ...props }) => (
          <div className="my-6 overflow-hidden rounded-2xl bg-slate-100">
            <img src={src} alt={alt || ''} className="w-full h-auto object-contain max-h-[400px]" loading="lazy" {...props} />
          </div>
        )
      }}
    >
  ...
  return (
    <div className="flex-1 bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col items-center justify-center min-w-[120px] transition-transform hover:scale-105">
      <div className="flex gap-1 mb-2">
        {[...Array(5)].map((_, i) => (
          <div key={i} className={`w-1.5 h-1.5 rounded-full ${i < level ? 'bg-emerald-600' : 'bg-slate-100'}`} />
        ))}
      </div>
    ...
    <>
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className="flex items-center gap-2 px-5 py-3 bg-slate-900 text-white border border-slate-700 rounded-2xl text-[10px] font-black uppercase hover:bg-slate-800 transition-all shadow-2xl"
        >
          <MapPin size={14} />
          Карта
        ...
        <>
          ...
          <div className="fixed bottom-24 right-6 bg-white border border-slate-200 rounded-[2rem] p-5 shadow-2xl z-50 w-80 max-h-[70vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setIsExpanded(false)} className="p-1 bg-slate-100 rounded-xl text-slate-400 hover:text-slate-600">
                <XCircle size={14} />
              </button>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`aspect-square rounded-xl flex items-center justify-center text-xs font-black transition-all hover:scale-110 ${
                      hasNoAnswer ? 'bg-slate-200 text-slate-400' :
...
export default function TeacherResultView() {
  ...
  useEffect(() => {
    const fetchResult = async () => {
      try {
        ...
        const res = await axios.get(`${API_BASE}/teacher/results/${resultId}`, {
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC]">
      <div className="text-center font-black uppercase tracking-widest text-slate-400 animate-pulse">Загрузка...</div>
    </div>
  );
      ...
      <div className="max-w-3xl mx-auto p-6 space-y-8">
        ...
        <header className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
          <div>
            ...
            <p className="text-[10px] font-black text-emerald-600 uppercase mt-3 tracking-[0.2em]">
                ...
                <div className="p-8">
                    ...
                    <div className="flex gap-8">
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="flex gap-0.5">
                          {[1, 2, 3, 4, 5].map((step) => (
                            <div key={step} className={`w-1 h-3 rounded-full transition-all ${
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {(Array.isArray(item.options) ? item.options : item.options.split(';'))
                          ...
                          .map((opt, i) => {
                            ...
                            return (
                              <div key={i} className={`p-5 rounded-2xl border-2 text-sm font-bold flex gap-4 ${cardStyle}`}>
                  ...
                  {item.solution && (
                    <div className="space-y-4">
                      <button onClick={() => toggleSolution(item.task_id)} className={`w-full py-5 rounded-2xl border-2 border-dashed flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-[0.25em] transition-all ${
                        isSolutionOpen ? 'bg-slate-900 border-slate-900 text-white shadow-lg' : 'border-slate-100 text-slate-400 hover:border-emerald-200 hover:text-emerald-600'
                      }`}>
                      ...
                      {isSolutionOpen && (
                        <div className="p-8 bg-slate-50 rounded-[2rem] border border-slate-100 animate-in slide-in-from-top-4 duration-500">
                          <div className="text-[9px] font-black text-emerald-600 uppercase mb-6 tracking-[0.3em] flex items-center gap-3">
                            <div className="w-8 h-px bg-emerald-600"></div> Решение
                          </div>
